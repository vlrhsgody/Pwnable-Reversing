...
Elf64_Shdr** read_section_headers(byte* elf_file, Elf64_Ehdr* elf_header) {
	...
	section_headers = (Elf64_Shdr **)malloc(sizeof(Elf64_Shdr *)*n_headers);
...
Elf64_Sym** get_symbol_table(byte* elf_file, Elf64_Shdr** section_headers, Elf64_Ehdr* elf_header) {
    ...
    for (int i = 0; i < n_symbols; ++i) {
        section_headers[i] = (Elf64_Sym*) malloc(sizeof(Elf64_Sym));
        memcpy(section_headers[i], elf_file + offset + i * elf_header->e_shentsize, sizeof(Elf64_Sym));
    }
    ...
    asdf    memcpy(symbol_table, elf_file + offset, size);
	return section_headers;
}
*/
...
//uint32 link = section_headers[i]->sh_link;
	...
	//link : symbol string table location index

    symbol_string_table = (byte*)malloc(sizeof(size));
